...
for hoja in excel.sheet_names:
    ...
    if clase[1] == '1' or clase[1] == 1 or "1" in clase[1] :
        flag1 += 1
    ...
    #obtuve el nombre de todos los atributos para la prueba de hipotesis
...
for i in range( len(X) ):
    ...
    try:
        X[i][np.isnan(X[i])] = 0
        Y[i][np.isnan(Y[i])] = 0
        Test = [["t estudent",stats.ttest_ind(X[i],Y[i])],["mannwhitneyu",stats.mannwhitneyu(X[i],Y[i])],["kruskal",stats.kruskal(X[i],Y[i])]]
#        print atributosX[i]
...
#        print "valor clase 2 : "+str(Y[i])
        ...
        ax.set_title(atributosX[i])
        ax.set_xlabel('Clases')
        ax.set_ylabel('Valores')
        ax.set_xticklabels(['Clase 1','Clase 2'])
        fig.savefig(str(atributosX[i])+'.png', bbox_inches='tight')
        plt.show()
